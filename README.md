...
### 분기 데이터
- 상단 기간 선택에서 "N분기"를 고르면 업종·스크리너·기업분석이 모두 해당 분기 기준(성장률은 전년 동기 대비)으로 바뀝니다.
- DART 갱신 시 분기보고서(1분기 `11013`, 반기 `11012`, 3분기 `11014`)도 함께 수집합니다 (`DART_QUARTERLY=true`). 손익은 당기 3개월 값, 재무상태는 분기말 잔액을 저장하고, 4분기는 연간 − 1~3분기로 계산합니다. 현금흐름은 분기 보고서에 누적값만 있어 연간만 제공합니다.
- 판관비 세부 항목은 분기에는 제공되지 않으며, 비상장사는 분기 공시가 없어 연간만 보입니다.
- 엑셀로도 분기를 넣을 수 있습니다. raw 시트에 `2025Q1`, `2025년 2분기` 같은 헤더 컬럼을 추가하면 해당 기간으로 임포트됩니다.
